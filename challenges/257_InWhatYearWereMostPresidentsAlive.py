## https://old.reddit.com/r/dailyprogrammer/comments/49aatn/20160307_challenge_257_easy_in_what_year_were/

from time import localtime


def most_alive_year(csv):
    end = localtime()[0]
    pres = [((c := l.split(","))[1].split()[2], x[2] if (x := c[3].split()) else end) for l in csv.split("\n")[1:]]
    pres = [(int(b), int(d)) for b, d in pres]
    start = pres[0][1]
    years = [0] * (end - start)
    for i in range(end - start):
        ...
        for p in pres:
            if p[0] <= y <= p[1]:
                years[i] += 1
    return start + years.index(max(years))
...
valid_year = [1822, 1823, 1824, 1825, 1826, 1831, 1833, 1834, 1835, 1836, 1837, 1838, 1839, 1840, 1841, 1843, 1844, 1845]

assert most_alive_year(input) in valid_year
