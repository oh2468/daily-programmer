## https://old.reddit.com/r/dailyprogrammer/comments/72ivih/20170926_challenge_333_easy_packet_assembler/

def assemble_packets(lines):
    messages = {}
    for line in lines.split("\n"):
        l = line.split()
        m_id, pn, tp = int(l[0]), int(l[1]), int(l[2])
        msg = " ".join(l[3:])
        if m_id not in messages:
            messages[m_id] = [0] * (tp + 1)
        messages[m_id][pn] = msg
        messages[m_id][-1] += 1
        if messages[m_id][-1] == tp:
            for i, mp in enumerate(messages[m_id][:-1]):
                print(f"{str(m_id).ljust(4)}    {str(i).ljust(4)}{str(tp).ljust(4)}{mp}")

ex_input = """6220    1   10  Because he's the hero Gotham deserves, 
6220    9   10   
...
ch_input = """7469    1   7   believe. Attack ships on fire off the 
...
assemble_packets(ex_input)
assemble_packets(ch_input)
